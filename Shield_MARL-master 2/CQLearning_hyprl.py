...
class CQLearning:
    def __init__(self, map_name='example', nagents=2, nactions=5, grid=False, alpha=1, disc=0.9,
        ...
        if not grid:
            self.env = GridEnv(nagents=nagents, map_name=map_name, norender=True, method="HypRL")  # set up ma environment
        else:
            self.env = GridEnv(nagents=nagents, map_name=map_name, norender=True, padding=True, method="HypRL")
    ...
    def initialize_qvalues(self, step_max=250, episode_max=200, noop=False, c_cost=10):
        ...
        for a in range(self.nagents):
            ...
            qval, hist = singleQL.run(single_env, step_max=step_max, episode_max=episode_max, discount=0.9,
                                      debug=False,noop=noop, save=True, N=self.nsaved, epsilon=0.8,c_cost=c_cost)
    ...
    def update(self, states, obs, rewards, actions):
        ...
        for i in range(self.nagents):
            ...
            elif self.is_dangerous(rewards, self.W2[i][states[i][0]][states[i][1]][actions[i]][:-1],
                                   self.W1[i][states[i][0]][states[i][1]][actions[i]][:-1]):
            ...
            if self.marks[i][states[i][0]][states[i][1]] == 0 or self.env.goal_flag[i]:
                ...
                if found and np.all(self.joint_marks[i][self.mark_index[i]][:-1] == joint_state):
                    ...
                    m_value = self.alpha * (rewards + self.discount * max(self.qvalues[i][obs[i][0]][obs[i][1]]))
                    # old val
        ...
        for i in range(self.nagents):
            if not self.env.goal_flag[i]:
                ...
                if np.isnan(nextval):  # check if there are still spaces left
                    ...
                    self.W1[i][p[0]][p[1]][actions[i]][int(index)] = rew
                    ...
                    self.W2[i][p[0]][p[1]][actions[i]][int(index2)] = rew
            ...
            for s in range(step_max):  # loop over steps
                ...
                for a in range(self.nagents):  # greedy select an action
                    if not testing:
                        ...
                        actions[a] = self.action_selection(pos, a, epsilon=0.0)
