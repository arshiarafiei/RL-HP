...
class CQLearning:
    def __init__(self, map_name='example', nagents=2, nactions=5, grid=False, alpha=1, disc=0.9,
        ...
        if not grid:
            self.env = GridEnv(nagents=nagents, map_name=map_name, norender=True)  # set up ma environment
        else:
            self.env = GridEnv(nagents=nagents, map_name=map_name, norender=True, padding=True)
