...
def main(tr):
    ...
    env = GridEnv(map_name='basic', nagents=NUM_AGENTS, norender=True, padding=True)
    ...
    for episode in range(1,NUM_EPISODES):
        ...
        print(f"basic (0,5) {tr} : Episode {episode + 1}/{NUM_EPISODES}, Total Reward: {total_reward}, Done: {done}, Done: {total_done}, Collision: {total_collision} ,Epsilon: {epsilon:.2f}")
        arr = [episode, total_done, total_collision,s]
        df.loc[len(df)] = arr
        st = "data/basic/"+str(tr)+".csv"
