...
class PPOAgent:
    def __init__(self, state_size, action_size, n_agent, learning_rate=0.001, gamma=0.85, epsilon_clip=0.2, entropy_beta=0.1):
...
def reward_fairness(env,tarjectories,step_size):
    ...
    for index in range(len(tarjectories[0])):
        ...
        phi_dist.append(15 - abs(tarjectories[0][index][2]-tarjectories[1][index][2]))
...
def train_ppo(env, agent, tries, episodes=1000, step_size=1000, batch_size=32, update_epochs=50):
    ...
    for e in range(episodes):
        ...
        st = "data/grid"+str(env.grid_size)+"new100/fair_"+str(env.grid_size)+"_"+str(tries)+".csv"
