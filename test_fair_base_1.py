...
class Job:
    ...
    def step(self, action):
        next_positions = []
        proposed_moves = {}  # Dictionary to count attempted moves
        
        # Step 1: Determine next positions
        for i, (x, y, alloc) in enumerate(self.state):
            if action[i] == 0:  # Move up
                ...
                new_x, new_y = x, y  # No movement

            next_positions.append((new_x, new_y, alloc))

            # Track how many agents want to move to this position
            proposed_moves[(new_x, new_y)] = proposed_moves.get((new_x, new_y), 0) + 1

        # Step 2: Resolve conflicts
        unique_positions = []
        for i, (x, y, alloc) in enumerate(next_positions):
            if proposed_moves[(x, y)] > 1 and (x, y) in [(p[0], p[1]) for p in self.state]:
                # If there's a conflict and an agent was already here, new agent gets pushed back
                unique_positions.append(self.state[i])  # Stay in original position
            else:
                unique_positions.append((x, y, alloc))

        # Step 3: Check if any agent reached the resource
        updated_state = []
...
class PPOAgent:
    def __init__(self, state_size, action_size, n_agent, learning_rate=0.001, gamma=0.8, epsilon_clip=0.2, entropy_beta=0.1):
...
def reward_fairness(env,tarjectories,step_size):
    ...
    for ag in range(env.n_agent):
        ...
        for tr in tarjectories[ag]:
            temp.append((int(env.grid_size/2))*2 - (2* env.calculate_distance(tr)))
    ...
    for index in range(len(tarjectories[0])):
        ...
        phi_dist.append(20 - abs(tarjectories[0][index][2]-tarjectories[1][index][2]))
...
def train_ppo(env, agent, tries, episodes=1000, step_size=1000, batch_size=32, update_epochs=50):
    ...
    for e in range(episodes):
        ...
        for step in range(step_size):
            ...
            if (next_state[0][0], next_state[0][1]) == env.resource or (next_state[1][0], next_state[1][1]) == env.resource:
                reward += 5
            if abs(next_state[0][2] -  next_state[1][2])>15:
                reward += -5
            ...
            if max(fairness_metric) < 0.3 and sum(allocation_total) > (step_size * 0.5):
                done = True
        ...
        st = "data/fair_base_3/"+str(tries)+".csv"
        ...
        for n in n_ag:
            for g in gr:
                ...
                for i in range(0,5):
                    ...
                    train_ppo(env, agent, i,episodes=500, step_size=100, batch_size=64, update_epochs=50)
